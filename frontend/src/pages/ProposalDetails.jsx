...
export function ProposalDetails() {
    ...
    const syncProposal = async () => {
        ...
        try {
            const res = await fetch(
                `${API_BASE}/proposals/${proposal._id}/sync`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            alert("Proposal synced with blockchain");
        } catch (err) {
    ...
    const advanceToStage3 = async () => {
        ...
        const confirmed = window.confirm(
            "This action is final. Bids will be locked and execution will be enabled. Continue?"
        );
        if (!confirmed) return;

        try {
            const res = await fetch(
                `${API_BASE}/proposals/${proposal._id}/advance-stage`,
                { method: "POST" }
            );
            ...
            alert("Failed to advance proposal: " + err.message);
        }
    };
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-gradient-to-r from-gray-900 to-maroon-900 p-1 rounded-3xl mt-8 shadow-xl">
                                <div className="bg-white rounded-[22px] p-6">
                                    ...
                                    {consensus && proposal.status_stage === "stage-2" && (
                                        <button
                                            onClick={advanceToStage3}
                                            className="mt-4 w-full bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg transition"
                                        >
                                            Finalize & Move to Stage 3
                                        </button>
                                    )}
